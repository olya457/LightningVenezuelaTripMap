import React, {useMemo, useState} from 'react';
import {Image, Pressable, Share, StyleSheet, Text, View} from 'react-native';
import {images} from '../assets/images';
import {AppScreen} from '../components/AppScreen';
import {Button} from '../components/Buttons';
import {quizQuestions} from '../data/quiz';
import {colors, shadow} from '../theme';

type QuizMode = 'intro' | 'playing' | 'results';

export function QuizScreen() {
  const [mode, setMode] = useState<QuizMode>('intro');
  const [index, setIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const question = quizQuestions[index];
  const selected = answers[question?.id];
  const score = useMemo(
    () =>
      quizQuestions.reduce(
        (total, item) => total + (answers[item.id] === item.answer ? 1 : 0),
        0,
      ),
    [answers],
  );

  const start = () => {
    setAnswers({});
    setIndex(0);
    setMode('playing');
  };

  const choose = (answer: string) => {
    setAnswers(current => ({...current, [question.id]: answer}));
  };

  const next = () => {
    if (index === quizQuestions.length - 1) {
      setMode('results');
      return;
    }
    setIndex(current => current + 1);
  };

  const shareResults = () => {
    Share.share({
      title: 'Lightning Knowledge Quiz',
      message: `I scored ${score}/${quizQuestions.length} in the Lightning Knowledge Quiz.`,
    });
  };

  if (mode === 'intro') {
    return (
      <AppScreen scroll>
        <View style={styles.intro}>
          <Image source={images.onboardingQuiz} style={styles.introImage} />
          <Text style={styles.introTitle}>Lightning Knowledge Quiz</Text>
          <Text style={styles.introBody}>
            Test your knowledge of lightning phenomena and Venezuelas storm science.
          </Text>
          <View style={styles.infoBox}>
            <Text style={styles.infoText}>❔  5 questions about lightning types</Text>
            <Text style={styles.infoText}>⏱️  15 seconds per question</Text>
            <Text style={styles.infoText}>☁️  Each question shows a lightning image</Text>
          </View>
          <Button
            title="Start Quiz"
            emoji="▶"
            variant="purple"
            onPress={start}
            style={styles.startButton}
          />
        </View>
      </AppScreen>
    );
  }

  if (mode === 'results') {
    const percent = Math.round((score / quizQuestions.length) * 100);

    return (
      <AppScreen title="Results" scroll>
        <View style={styles.resultHeader}>
          <View style={styles.scoreCircle}>
            <Text style={styles.scoreText}>{score}/{quizQuestions.length}</Text>
            <Text style={styles.scorePercent}>{percent}%</Text>
          </View>
          <Text style={styles.stars}>{score > 3 ? '⭐ ⭐ ⭐' : '⭐ ☆ ☆'}</Text>
          <Text style={styles.keep}>{score > 3 ? 'Storm Expert!' : 'Keep Charging!'}</Text>
        </View>
        {quizQuestions.map(item => {
          const correct = answers[item.id] === item.answer;
          return (
            <View key={item.id} style={styles.resultRow}>
              <Text style={styles.resultQuestion}>{correct ? '✅' : '⭕'} {item.question}</Text>
              <Text style={styles.resultAnswer}>✓ {item.answer}</Text>
            </View>
          );
        })}
        <View style={styles.resultActions}>
          <Button
            title="Back"
            variant="ghost"
            onPress={() => setMode('intro')}
            style={styles.resultButton}
          />
          <Button
            title="Share"
            onPress={shareResults}
            style={styles.resultButton}
          />
        </View>
      </AppScreen>
    );
  }

  return (
    <AppScreen scroll>
      <View style={styles.progressRow}>
        <Text style={styles.progressText}>{index + 1} / {quizQuestions.length}</Text>
        <View style={styles.progressTrack}>
          <View
            style={[
              styles.progressFill,
              {width: `${((index + 1) / quizQuestions.length) * 100}%`},
            ]}
          />
        </View>
        <View style={styles.timer}>
          <Text style={styles.timerText}>14</Text>
        </View>
      </View>
      <Image source={question.image} style={styles.questionImage} />
      <Text style={styles.question}>{question.question}</Text>
      <View style={styles.options}>
        {question.options.map((option, optionIndex) => {
          const isSelected = selected === option;
          const answered = Boolean(selected);
          const correct = option === question.answer;
          return (
            <Pressable
              key={option}
              onPress={() => choose(option)}
              disabled={answered}
              style={[
                styles.option,
                isSelected && styles.selectedOption,
                answered && correct && styles.correctOption,
                answered && isSelected && !correct && styles.wrongOption,
              ]}>
              <View style={styles.optionBadge}>
                <Text style={styles.optionBadgeText}>
                  {String.fromCharCode(65 + optionIndex)}
                </Text>
              </View>
              <Text style={styles.optionText}>{option}</Text>
            </Pressable>
          );
        })}
      </View>
      <Button
        title={index === quizQuestions.length - 1 ? 'Result' : 'Next question'}
        disabled={!selected}
        onPress={next}
        style={styles.nextButton}
      />
    </AppScreen>
  );
}

const styles = StyleSheet.create({
  intro: {
    alignItems: 'center',
    paddingTop: 8,
  },
  introImage: {
    width: '98%',
    maxWidth: 340,
    aspectRatio: 1,
    borderRadius: 22,
    marginBottom: 24,
    ...shadow,
  },
  introTitle: {
    color: colors.text,
    fontSize: 22,
    fontWeight: '900',
    textAlign: 'center',
  },
  introBody: {
    color: colors.muted,
    fontSize: 14,
    lineHeight: 21,
    fontWeight: '700',
    textAlign: 'center',
    marginTop: 10,
  },
  infoBox: {
    width: '100%',
    borderRadius: 16,
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: colors.panel,
    padding: 16,
    marginTop: 28,
  },
  infoText: {
    color: '#aab8d8',
    fontSize: 13,
    fontWeight: '800',
    marginVertical: 6,
  },
  startButton: {
    width: '100%',
    marginTop: 24,
  },
  progressRow: {
    marginTop: 10,
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 18,
  },
  progressText: {
    color: colors.violet,
    fontSize: 12,
    fontWeight: '900',
    width: 46,
  },
  progressTrack: {
    flex: 1,
    height: 2,
    backgroundColor: '#16234a',
    marginHorizontal: 12,
  },
  progressFill: {
    height: 2,
    backgroundColor: colors.violet,
  },
  timer: {
    width: 48,
    height: 48,
    borderRadius: 24,
    borderWidth: 3,
    borderColor: colors.cyan,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: colors.panel,
  },
  timerText: {
    color: colors.cyan,
    fontWeight: '900',
    fontSize: 15,
  },
  questionImage: {
    width: '100%',
    height: 170,
    borderRadius: 14,
    marginBottom: 16,
  },
  question: {
    color: colors.text,
    fontSize: 15,
    lineHeight: 21,
    fontWeight: '900',
    marginBottom: 12,
  },
  options: {
    marginBottom: 22,
  },
  option: {
    minHeight: 52,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: colors.panel,
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 12,
    marginBottom: 10,
  },
  selectedOption: {
    borderColor: colors.borderStrong,
  },
  correctOption: {
    backgroundColor: '#0d5228',
    borderColor: colors.green,
  },
  wrongOption: {
    backgroundColor: '#651521',
    borderColor: colors.danger,
  },
  optionBadge: {
    width: 22,
    height: 22,
    borderRadius: 11,
    backgroundColor: '#122b59',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 10,
  },
  optionBadgeText: {
    color: '#4f91ff',
    fontSize: 10,
    fontWeight: '900',
  },
  optionText: {
    flex: 1,
    color: colors.text,
    fontSize: 13,
    fontWeight: '800',
  },
  nextButton: {
    marginTop: 4,
  },
  resultHeader: {
    alignItems: 'center',
    marginBottom: 24,
  },
  scoreCircle: {
    width: 88,
    height: 88,
    borderRadius: 44,
    borderWidth: 6,
    borderColor: colors.violet,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: colors.panel,
    marginBottom: 18,
  },
  scoreText: {
    color: colors.text,
    fontSize: 23,
    fontWeight: '900',
  },
  scorePercent: {
    color: colors.dim,
    fontSize: 10,
    fontWeight: '900',
  },
  stars: {
    color: colors.yellow,
    fontSize: 22,
    marginBottom: 8,
  },
  keep: {
    color: colors.violet,
    fontWeight: '900',
  },
  resultRow: {
    borderRadius: 12,
    borderWidth: 1,
    borderColor: '#3d1930',
    backgroundColor: colors.panel,
    padding: 14,
    marginBottom: 10,
  },
  resultQuestion: {
    color: '#b7c3df',
    fontSize: 12,
    lineHeight: 18,
    fontWeight: '800',
  },
  resultAnswer: {
    color: colors.green,
    fontSize: 12,
    fontWeight: '900',
    marginTop: 8,
  },
  resultActions: {
    flexDirection: 'row',
    marginTop: 18,
  },
  resultButton: {
    flex: 1,
    marginRight: 10,
  },
});
